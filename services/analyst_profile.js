...
// Returns everything an instructor needs to evaluate one student:
//   - Core stats, performance score, streak
//   - Per-lab breakdown with question-level detail
//   - Weak areas (stuck questions + under-performing categories)
//   - Alert triage history
...
function getAnalystProfile(userId) {
  ...
  const labStats = db.prepare(
    `SELECT
       COUNT(DISTINCT up.lab_id) FILTER (WHERE up.status='completed') AS labs_done,
       COUNT(DISTINCT up.lab_id) FILTER (WHERE up.status='in_progress') AS labs_progress,
  ...
  const totalScore  = (labStats.lab_pts || 0) + alertPts;
  ...
  const labActivity = allLabs.map(lab => {
    ...
    // Time spent: from first answer to last answer (or started_at to completed_at)
    let timeSpentMins = null;
    if (lab.started_at && lab.completed_at) {
      timeSpentMins = Math.round(
        (new Date(lab.completed_at) - new Date(lab.started_at)) / 60000
      );
    } else {
  ...
  // ── Weak areas ────────────────────────────────────────────────────────
  // Questions the student is stuck on: attempted but not correct
  const stuckQuestions = labActivity.flatMap(lab =>
    lab.questions
  ...
  ).sort((a, b) => b.wrong_count - a.wrong_count).slice(0, 20);

  // Category-level performance
  const catMap = {};
  labActivity.forEach(lab => {
    const cat = lab.category || 'Uncategorized';
  ...
  const categoryPerformance = Object.entries(catMap).map(([cat, d]) => ({
    category:    cat,
    lab_count:   d.lab_count,
    labs_done:   d.labs_done,
    correct:     d.correct,
    attempted:   d.total,
  ...
  const alertHistory = db.prepare(
    `SELECT ac.alert_id, sa.title AS alert_title, sa.severity, sa.category,
            ...
            ac.investigation_score, ac.triage_reason, ac.fp_reason,
            ac.step_scores, ac.scoring_feedback, ac.closed_at
  ...
  // ── Activity feed (chronological, last 50 events) ─────────────────────
  ...
  const labStartEvents = db.prepare(
    `SELECT up.started_at AS ts, 'lab_start' AS type, 'started' AS result,
            0 AS pts, l.title AS detail, l.slug AS lab_slug, l.title AS lab_title
     FROM user_progress up JOIN labs l ON l.id=up.lab_id
     WHERE up.user_id=? AND up.started_at IS NOT NULL
     ORDER BY up.started_at DESC LIMIT 15`
  ).all(userId);

  const labCompleteEvents = db.prepare(
    `SELECT up.completed_at AS ts, 'lab_complete' AS type, 'completed' AS result,
     ...
     ORDER BY up.completed_at DESC LIMIT 15`
  ).all(userId);

  const allEvents = [...answerEvents, ...alertEvents, ...labStartEvents, ...labCompleteEvents]
    .filter(e => e.ts)
