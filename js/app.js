// NEXUS SOC Training Platform — Core App Engine
...
const App = (function() {
  ...
  function addScore(pts, label) {
    ...
    // Bump leaderboard
    const me = NEXUS_DATA.analysts.find(a => a.id === state.analystId);
    if (me) me.score = state.score;
    _sortLeaderboard();
  }

  function _sortLeaderboard() {
    NEXUS_DATA.analysts.sort((a,b) => b.score - a.score);
    NEXUS_DATA.analysts.forEach((a,i) => a.rank = i+1);
  }
  ...
  function startLiveTicker() {
    ...
    if (!ticker) return;
    const items = NEXUS_DATA.alerts.filter(a => a.severity === 'critical' || a.severity === 'high');
  ...
  function updateNavBadges() {
    const unreviewedCount = NEXUS_DATA.alerts.filter(a => !state.solvedAlerts.has(a.id)).length;
    document.querySelectorAll('.badge-alerts').forEach(el => el.textContent = unreviewedCount);
    const openCases = NEXUS_DATA.cases.filter(c => c.status === 'open').length;
  ...
  function mitreHtml(id) {
    const t = NEXUS_DATA.mitre_techniques[id];
...
const Dashboard = (function() {
  ...
  function renderStats() {
    const total    = NEXUS_DATA.alerts.length;
    const critical = NEXUS_DATA.alerts.filter(a=>a.severity==='critical').length;
    const high     = NEXUS_DATA.alerts.filter(a=>a.severity==='high').length;
    const openCases = NEXUS_DATA.cases.filter(c=>c.status==='open').length;
    const solved    = App.state.solvedAlerts.size;
    const tp        = NEXUS_DATA.alerts.filter(a=>App.state.solvedAlerts.has(a.id) && a.answer==='TP').length;
  ...
  function renderSeverityChart() {
    ...
    NEXUS_DATA.alerts.forEach(a => { if (counts[a.severity]!==undefined) counts[a.severity]++; });
  ...
  function renderMitreBars() {
    ...
    if (!el) return;
    const counts = {};
    NEXUS_DATA.alerts.forEach(a => { counts[a.mitre] = (counts[a.mitre]||0)+1; });
    const sorted = Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,6);
    const max = sorted[0] ? sorted[0][1] : 1;
    el.innerHTML = sorted.map(([tid,count]) => {
      const t = NEXUS_DATA.mitre_techniques[tid];
  ...
  function renderRecentAlerts() {
    ...
    if (!el) return;
    const recent = NEXUS_DATA.alerts.slice(0,8);
...
const Alerts = (function() {
  ...
  function applyFilters() {
    ...
    filtered = NEXUS_DATA.alerts.filter(a => {
  ...
  function openDetail(id) {
    const alert = NEXUS_DATA.alerts.find(a => a.id === id);
    if (!alert) return;
    selected = alert;
    ...
    const t = NEXUS_DATA.mitre_techniques[alert.mitre];
...
const SIEM = (function() {
  ...
  function runQuery(q) {
    ...
    results = NEXUS_DATA.siem_logs.filter(log => {
  ...
  function showLog(id) {
    const log = NEXUS_DATA.siem_logs.find(l => l.id === id);
    if (!log) return;
    currentLog = log;
...
const Cases = (function() {
  ...
  function renderList() {
    ...
    if (!el) return;
    el.innerHTML = NEXUS_DATA.cases.map(c => {
  ...
  function openCase(id) {
    const c = NEXUS_DATA.cases.find(x=>x.id===id);
    if (!c) return;
    selectedCase = c;
  ...
  function toggleTask(caseId, taskId) {
    const c = NEXUS_DATA.cases.find(x=>x.id===caseId);
    if (!c) return;
...
const Intel = (function() {
  ...
  function renderIocTable() {
    ...
    if (!el) return;
    el.innerHTML = NEXUS_DATA.iocs.map(ioc => {
      const pct = ioc.score;
  ...
  function lookupIoc(value) {
    const ioc = NEXUS_DATA.iocs.find(i => i.value === value);
    if (!ioc) { App.toast('IOC not found in NEXUS threat database', 'warning'); return; }
    const modal = document.getElementById('modal-ioc-detail');
    if (!modal) return;
    const pct = ioc.score;
    const barColor = pct >= 80 ? 'var(--critical)' : pct >= 50 ? 'var(--high)' : pct >= 20 ? 'var(--medium)' : 'var(--success)';
    const verdict = ioc.malicious
      ? `<div class="banner banner-critical" style="margin-top:0">⚠ MALICIOUS — NEXUS Threat Score: ${pct}/100</div>`
...
const Leaderboard = (function() {
  function init() {
    ...
    NEXUS_DATA.analysts.forEach((a,i) => {
  ...
  function renderBoard() {
    ...
    if (!el) return;
    const badges = ['👑','🥈','🥉'];
    el.innerHTML = NEXUS_DATA.analysts.map((a,i) => {
  ...
  function renderMyStats() {
    ...
    if (!el) return;
    const tp = NEXUS_DATA.alerts.filter(a=>App.state.solvedAlerts.has(a.id)&&a.answer==='TP').length;
    const fp = NEXUS_DATA.alerts.filter(a=>App.state.solvedAlerts.has(a.id)&&a.answer==='FP').length;
    const tasks = NEXUS_DATA.cases.flatMap(c=>c.tasks).filter(t=>t.status==='closed').length;
    ...
    el.innerHTML = `
      ...
      <div class="stat-card accent">
        ...
        <div class="stat-value">${(NEXUS_DATA.analysts.find(a=>a.id===App.state.analystId)?.rank) || '—'}</div>
        <div class="stat-delta">of ${NEXUS_DATA.analysts.length} analysts</div>
      </div>`;
  }
