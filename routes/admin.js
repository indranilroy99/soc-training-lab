...
async function updateUser(req, res, userId) {
  ...
  if (body.active !== undefined) {
    db.prepare(`UPDATE users SET is_active=? WHERE id=?`).run(body.active ? 1 : 0, uid);
  }
  if (body.password) {
...
function listAdminLabs(req, res) {
  ...
  const labs = db.prepare(
    ...
    ...l,
    alert_refs: l.alert_refs ? JSON.parse(l.alert_refs) : [],
    evidence:   (() => { try { return l.evidence ? JSON.parse(l.evidence) : []; } catch { return []; } })(),
  }));
  return ok(res, labs);
}
...
function getAnalystActivity(req, res, userId) {
  ...
  return ok(res, {
    ...
    fp_accuracy:    fps.length > 0 ? Math.round((fp_correct / fps.length) * 100) + '%' : 'N/A',
    ...
    records: closures.map(c => ({
      alert_id:            c.alert_id, alert_title: c.alert_title,
      severity:            c.severity, classification: c.classification,
      is_correct:          !!c.is_correct, points: c.points_awarded,
      ...
      containment_steps:   c.containment_steps, eradication_steps: c.eradication_steps,
      recovery_steps:      c.recovery_steps, rca_notes: c.rca_notes,
      closed_at:           c.closed_at,
    })),
  });
}

module.exports = { getStats, listUsers, createUser, updateUser, deleteUser, getProgress, listAdminLabs, getAnalystActivity };
